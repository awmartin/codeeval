...
std::string TaskRunner::evaluate(std::string line) {
  std::vector<int64_t> values = Helpers::parse_list_of_integers(line, std::string(" "));
  int64_t n = values.at(0);
  
  // Initialize the vector of ints to store how many counts we have.
  std::vector<int64_t> found;
  for (int64_t i = 0; i < n - 1; i ++) {
    found.push_back(0);
  }
  
  // Check the differences and register the counts.
  int64_t s = 0;
  for (int64_t i = 1; i < values.size() - 1; i++) {
    int64_t diff = abs(values.at(i) - values.at(i + 1));
    found[diff - 1]++;
  }
  
  // Check for all ones.
  bool jolly = true;
  std::vector<int64_t>::iterator it = found.begin();
  for (; it != found.end(); ++it) {
    if (*it != 1) {
      jolly = false;
      break;
  ...
  if (jolly) {
    return std::string("Jolly");
  } else {
    return std::string("Not jolly");
  }
}
