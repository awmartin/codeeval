...
std::string TaskRunner::evaluate(std::string line) {
  ...
  for(; it != line.end(); ++it) {
    if (*it == '(') {
      ...
      stack.push_back(BRACES);
    }

    
    if (*it == ')') {
      if (stack.size() == 0) { return f; }
      if (stack.back() != PARENS) { return f; }
      stack.pop_back();
    } else if (*it  == ']') {
      if (stack.size() == 0) { return f; }
      if (stack.back() != BRACKETS) { return f; }
      stack.pop_back();
    } else if (*it == '}') {
      if (stack.size() == 0) { return f; }
      if (stack.back() != BRACES) { return f; }
      stack.pop_back();
    }
  }
...
std::vector<std::string> TaskRunner::prepend(
    std::string item,
    std::vector<std::string> items
    ) {
...
std::vector<std::string> TaskRunner::exclude(
    std::string element,
    std::vector<std::string> elements
    ) {
  std::vector<std::string> result;
