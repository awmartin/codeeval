...
std::string TaskRunner::evaluate(std::string& line) {
  ...
  int64_t n = atoi(input[0].c_str());
  std::vector<int64_t> values = Helpers::parse_list_of_integers(input[1], std::string(","));
  ...
  int64_t duplicate_value = -1;
  std::vector<int64_t>::iterator it = values.begin();
  ...
  return std::to_string(duplicate_value);
}
