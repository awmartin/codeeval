...
std::string TaskRunner::evaluate(std::string& line) {
  std::vector<std::string> elements = Helpers::split_string(line, std::string(" "));
  int64_t m = atoi(elements.back().c_str());
  int64_t num_elements = elements.size() - 1;
  if (m > num_elements) return std::string("");
  
  return elements[num_elements - m];
}
