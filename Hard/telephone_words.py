...
class TelephoneWords(object):
    ...
    def compute(self, phone_number):
        words = sorted(self.permutations(phone_number))
        return ",".join(words)
        
    def permutations(self, phone_number):
        results = []
        ...
        remaining_permutations = self.permutations(phone_number[1:])
        for permutation in remaining_permutations:
            for letter in letters[first]:
                results.append(letter + permutation)
        return results
